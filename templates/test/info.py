...
# q:i need to run this code in python
# a:you can run this code in python by using the following command
...
def load_local_data(localData):
    ...
    for file in local_directory_list:
        # Split the file name at underscore, and take the last part as emotion.
        emotion = file.split('_')[-1].replace('.wav', '')
        # Append the emotion to the file_emotion list.
        file_emotion.append(emotion)
        # Construct the full file path and append it to the file_path list.
        file_path.append(localData + file)
    # Create two dataframes from the file_emotion and file_path lists.
...
def upload_file():
    ...
    if button == 'Submit':
        ...
        return "File saved and data appended to CSV file."